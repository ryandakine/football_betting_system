...
class UnifiedAIProvider:
    ...
    def get_consensus_analysis(self, game_data):
        return {"consensus": "odds_consensus", "confidence": 0.5}
    
    def get_consensus_with_fallback(self, game):
        """Derive a simple real-data consensus from H2H odds (no mock picks).

        - Uses bookmaker 'h2h' markets and decimal prices
        - Implied probability = 1/price
        - Picks team with higher average implied probability across books
        """
        try:
            ...
            if not bookmakers or not home or not away:
                return {
                    'consensus': {'team': 'Unknown', 'confidence': 0.0, 'strength': 'insufficient_data'},
                    'individual_analyses': [],
                    'active_providers': 0,
                    'fallback_used': False,
                }
            ...
            for bk in bookmakers:
                for market in bk.get('markets', []):
                    ...
                    # Map prices to our home/away teams (case-insensitive)
                    p_home = None
                    p_away = None
                    for oc in outcomes:
                        ...
                        if ln == home_lower:
                            ...
                            p_away = prob
                    # If we couldn't match by names, try best two outcomes
                    if p_home is None or p_away is None:
                        if len(outcomes) >= 2:
                            # Pick top two by price (lowest price => higher implied prob)
                            try:
                                sorted_oc = sorted(
                                    [oc for oc in outcomes if oc.get('price')], key=lambda x: float(x['price'])
                                )
                                # First is favourite (higher prob)
                                fav = sorted_oc[0]
                                dog = sorted_oc[1]
                                fav_prob = 1.0 / float(fav['price'])
                                dog_prob = 1.0 / float(dog['price'])
                                # Assign by string containment if exact match failed
                                fav_name = (fav.get('name') or '').lower()
                                dog_name = (dog.get('name') or '').lower()
                                if p_home is None and (home_lower in fav_name or fav_name in home_lower):
                                    p_home = fav_prob
                                if p_away is None and (away_lower in fav_name or fav_name in away_lower):
                                    p_away = fav_prob
                                if p_home is None and (home_lower in dog_name or dog_name in home_lower):
                                    p_home = dog_prob
                                if p_away is None and (away_lower in dog_name or dog_name in away_lower):
                                    p_away = dog_prob
                                # If still unmapped, skip this book
                            except Exception:
                                pass
                    if p_home is None or p_away is None:
                        continue
                    ...
                    count += 1
                    per_book.append({
                        'provider': bk.get('key', 'book'),
                        'confidence': max(p_home, p_away),
                        'prediction': home if p_home >= p_away else away,
                    })

            # If no usable h2h found, try a simple favorite-from-h2h approach first
            if count == 0:
                best_name = None
                best_prob = 0.0
                h2h_books = 0
                for bk in bookmakers:
                    for market in bk.get('markets', []):
                        ...
                        for oc in (market.get('outcomes', []) or []):
                            ...
                            if prob > best_prob:
                                best_prob = prob
                                best_name = (oc.get('name') or '').strip()
                if best_name:
                    return {
                        'consensus': {
                            'team': best_name,
                            'confidence': float(max(0.0, min(1.0, best_prob))),
                            'strength': 'odds_favorite',
                        },
                        'individual_analyses': [],
                        'active_providers': h2h_books,
                        'fallback_used': False,
                    }

                # Fall back to spreads where available
                for bk in bookmakers:
                    for market in bk.get('markets', []):
                        if market.get('key') != 'spreads':
                            continue
                        ...
                        for oc in outcomes:
                            ...
                            if name and pt is not None:
                                if name == home_lower:
                                    ...
                                    away_line = float(pt)
                        # Use spread: negative line => favourite
                        if home_line is not None and away_line is not None:
                            fav = home if home_line < away_line else away
                            margin = abs(home_line - away_line)
                            conf = max(0.5, min(0.9, 0.55 + margin / 20.0))
                            return {
                                'consensus': {
                                    'team': fav,
                                    'confidence': float(conf),
                                    'strength': 'spread_consensus',
                                },
                                'individual_analyses': [],
                                'active_providers': 1,
                                'fallback_used': False,
                ...
                return {
                    'consensus': {'team': 'Unknown', 'confidence': 0.0, 'strength': 'no_h2h_or_spread'},
                    'individual_analyses': [],
                    'active_providers': 0,
                    'fallback_used': False,
                }

            avg_home = total_home_p / count
            avg_away = total_away_p / count
            ...
            return {
                'consensus': {
                    ...
                    'strength': 'odds_consensus',
                },
                'individual_analyses': per_book,
                'active_providers': count,
                'fallback_used': False,
            }
        except Exception:
            return {
                'consensus': {'team': 'Unknown', 'confidence': 0.0, 'strength': 'error'},
                'individual_analyses': [],
                'active_providers': 0,
                'fallback_used': False,
            }
...
class FootballMasterGUI:
    ...
    def __init__(self):
        ...
        self.current_sport = "nfl"  # Default to NFL for Sunday slates
        self.all_games = []
        self.predictions = {}
        self.parlays = []
    ...
    def open_daily_pulse(self):
        """Open the Daily Pulse viewer window."""
        try:
            DailyPulseWindow(self.root, bg_color=self.bg_color, fg_color=self.fg_color, accent_color=self.accent_color)
        except Exception as e:
            logger.error(f"Failed to open Daily Pulse viewer: {e}")
            try:
                messagebox.showerror("Daily Pulse", f"Failed to open Daily Pulse: {e}")
            except Exception:
                pass


class DailyPulseWindow(tk.Toplevel):
    """Lightweight viewer for Daily Pulse report with scrolling and auto-refresh."""

    def __init__(self, master, bg_color="#111", fg_color="#eee", accent_color="#00ffaa"):
        super().__init__(master)
        from pathlib import Path  # local import to avoid module-wide import changes
        from datetime import datetime  # local import
        self.title("📊 Daily Pulse")
    ...
    def _build_ui(self):
        ...
        tk.Label(top, text="Date:", bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT)
        self.date_var = tk.StringVar(value=self.current_date)
        self.date_menu = ttk.Combobox(top, textvariable=self.date_var, state="readonly", width=18)
        self.date_menu.pack(side=tk.LEFT, padx=6)
        self.date_menu.bind("<<ComboboxSelected>>", lambda e: self._load_report())
        self._refresh_date_options()

        tk.Button(top, text="Reload", command=self._load_report, bg="#2a2a2a", fg=self.fg_color).pack(side=tk.LEFT, padx=6)
        ...
        vs = ttk.Scrollbar(outer, orient="vertical", command=self.canvas.yview)
    ...
    def _refresh_date_options(self):
        ...
        if self.reports_dir.exists():
            for p in sorted(self.reports_dir.glob("daily_report_*.json"), reverse=True):
                try:
                    d = p.stem.replace("daily_report_", "")
                    dates.append(d)
                except Exception:
                    continue
        if not dates:
    ...
    def _load_report(self):
        ...
        header = tk.Label(self.content, text=f"Daily Pulse — {date_str}", font=("Arial", 16, "bold"), bg=self.bg_color, fg=self.accent_color)
        header.pack(anchor="w", pady=(0, 8))

        if not json_path.exists():
            tk.Label(self.content, text="No report found for this date.", bg=self.bg_color, fg=self.fg_color).pack(anchor="w")
            tk.Label(self.content, text="Tip: Run scripts/run_daily_report.py to generate.", bg=self.bg_color, fg=self.fg_color).pack(anchor="w", pady=(4,0))
            return
        ...
        try:
            ...
            tk.Label(self.content, text=f"Failed to load JSON: {e}", bg=self.bg_color, fg=self.fg_color).pack(anchor="w")

        if txt_path.exists():
            sep = tk.Frame(self.content, height=1, bg="#333"); sep.pack(fill=tk.X, pady=8)
            tk.Label(self.content, text="Text Summary", font=("Arial", 14, "bold"), bg=self.bg_color, fg=self.accent_color).pack(anchor="w")
            text_box = scrolledtext.ScrolledText(self.content, height=12, wrap=tk.WORD)
            text_box.pack(fill=tk.BOTH, expand=True, pady=4)
            try:
                text_box.insert(tk.END, txt_path.read_text())
            except Exception as e:
                text_box.insert(tk.END, f"Failed to read text summary: {e}")
            text_box.configure(state=tk.DISABLED)

    def _render_sections(self, data: dict):
        def section(title: str):
            tk.Label(self.content, text=title, font=("Arial", 14, "bold"), bg=self.bg_color, fg=self.accent_color).pack(anchor="w", pady=(8, 2))

        # MLB predictions
        mlb = data.get("mlb_predictions", {})
        section("⚾ MLB Predictions")
        if mlb.get("status") == "ok":
            stats = [
                f"Total: {mlb.get('total_predictions', 0)}",
                f"Value Bets: {mlb.get('value_bets', 0)}",
                f"Avg Confidence: {mlb.get('average_confidence', 0.0):.2f}",
                f"Avg Edge: {mlb.get('average_edge', 0.0):.3f}",
            ]
            tk.Label(self.content, text="  • " + " | ".join(stats), bg=self.bg_color, fg=self.fg_color).pack(anchor="w")
            tvb = mlb.get("top_value_bets", [])
            if tvb:
                for p in tvb:
                    line = f"  - {p.get('home_team','?')} vs {p.get('away_team','?')} → {p.get('predicted_winner','?')} (c={p.get('confidence',0):.2f}, e={p.get('edge',0):.3f})"
                    tk.Label(self.content, text=line, bg=self.bg_color, fg=self.fg_color).pack(anchor="w")
        else:
            tk.Label(self.content, text=f"{mlb.get('status','missing')}: {mlb.get('message','')} ", bg=self.bg_color, fg=self.fg_color).pack(anchor="w")
        ...
        if learning.get("status") == "ok":
            ...
            if summary:
                ...
                line = f"Total: {om.get('total_predictions','?')} | Accuracy: {om.get('accuracy','?')} | ROI: {om.get('roi','?')} | Trend: {summary.get('recent_trend','?')}"
                tk.Label(self.content, text=line, bg=self.bg_color, fg=self.fg_color).pack(anchor="w")
            else:
                tk.Label(self.content, text="Enhanced insights available (feedback system)", bg=self.bg_color, fg=self.fg_color).pack(anchor="w")
        else:
            tk.Label(self.content, text=f"{learning.get('status','missing')}: {learning.get('message','')} ", bg=self.bg_color, fg=self.fg_color).pack(anchor="w")

        # NFL tracking
        nfl = data.get("nfl_tracking", {})
        section("🏈 NFL Tracking")
        if nfl.get("status") == "ok":
            line = (
                f"Active Games: {nfl.get('active_games',0)} | Recent Predictions: {nfl.get('recent_predictions',0)} "
                f"| Recent Accuracy: {nfl.get('recent_accuracy',0.0):.2%} | Avg Confidence: {nfl.get('avg_confidence',0.0):.2f}"
            )
            tk.Label(self.content, text=line, bg=self.bg_color, fg=self.fg_color).pack(anchor="w")
        else:
            tk.Label(self.content, text=f"{nfl.get('status','missing')}: {nfl.get('message','')} ", bg=self.bg_color, fg=self.fg_color).pack(anchor="w")

    def _schedule_poll(self):
        try:
            self.after(self.poll_ms, self._poll_newer)
        except Exception:
            pass

    def _poll_newer(self):
        try:
            self._refresh_date_options()
            # If latest date changed to be newer than current selection, auto-switch
            if self.date_menu["values"]:
                latest = sorted(self.date_menu["values"], reverse=True)[0]
                if latest != self.date_var.get():
                    self.date_var.set(latest)
                    self._load_report()
        except Exception as e:
            logger.warning(f"DailyPulse poll error: {e}")
        finally:
            self._schedule_poll()
    ...
    def _create_top_bar(self):
        ...
        # Daily Pulse (system pulse/report) viewer
        tk.Button(
            button_frame,
            text="📊 Daily Pulse",
            command=self.open_daily_pulse,
            bg="#2a2a2a",
            fg=self.fg_color,
            font=("Arial", 12, "bold")
        ).pack(side=tk.LEFT, padx=5)
        
    def _create_quick_bet_tab(self):
        """Create a minimal Quick Bet tab to launch core actions quickly"""
        quick_frame = tk.Frame(self.notebook, bg=self.bg_color)
        self.notebook.add(quick_frame, text="⚡ Quick Bet")

        header = tk.Label(
            quick_frame,
            text="Quick Bet Controls",
            font=("Arial", 16, "bold"),
            bg=self.bg_color,
            fg=self.accent_color,
        )
        ...
        controls = tk.Frame(quick_frame, bg=self.bg_color)
        ...
        info = tk.Label(
            quick_frame,
            text="Use these controls to quickly refresh data, run predictions, and generate bet recommendations.",
            bg=self.bg_color,
            fg=self.fg_color,
        )
        info.pack(pady=10)
...
• Risk-adjusted target: 18.9%
    ...
    def _refresh_all_data(self):
        ...
        self._update_status("Refreshing live games and odds...")
        import threading
    ...
    async def _fetch_all_data(self):
        ...
        async with FootballOddsFetcher(
            ...
            # If live game API returned no games but odds contain real games,
            # synthesize minimal live game records from real odds so UI and predictions can run
            if (not getattr(game_data, "games", [])) and getattr(odds_data, "games", []):
                try:
                    from football_game_data_fetcher import FootballGameData, LiveGameData
                    synthesized = FootballGameData()
                    for g in odds_data.games:
                        synthesized.games.append(
                            LiveGameData(
                                game_id=g.game_id,
            ...
            self.all_games = enhanced_games or enriched_games or merged_games
            # Auto-run predictions when we have games
            if self.all_games:
                self.root.after(0, self.predict_all_games)
    ...
    def _on_predictions_complete(self, prediction_results: Dict[str, Any]):
        ...
        # Treat a prediction as successful if it's recorded in predictions_db for that game
        game_ids = list(prediction_results.keys())
        recorded_success = 0
        for gid in game_ids:
            try:
                rec = self.prediction_tracker.predictions_db.get(gid, {})
                if rec and rec.get('predictions'):
                    recorded_success += 1
            except Exception:
                pass

        successful_predictions = recorded_success
        failed_predictions = max(0, len(prediction_results) - successful_predictions)
        ...
        fallback_used = any(
            isinstance(r, dict) and r.get('fallback_used', False)
        ...
        # Populate the All Predictions text area for immediate visibility
        try:
            if hasattr(self, 'all_predictions_text'):
                self.all_predictions_text.delete(1.0, tk.END)
                # Collate latest prediction per game for display as "Away @ Home – Pick (confidence)"
                lines = []
                for gid in game_ids:
                    rec = self.prediction_tracker.predictions_db.get(gid, {})
                    ...
                    team = consensus.get('team', 'Unknown')
                    conf = consensus.get('confidence', 0.0)
                    home = rec.get('home_team') or 'Home'
                    away = rec.get('away_team') or 'Away'
                    lines.append(f"{away} @ {home} – {team} ({conf:.1%})")
                if lines:
                    self.all_predictions_text.insert(tk.END, "\n".join(lines))
                else:
                    # Fallback: show raw recorded predictions so the panel is never blank
                    try:
                        summary = {gid: self.prediction_tracker.predictions_db.get(gid, {}) for gid in game_ids}
                        self.all_predictions_text.insert(tk.END, json.dumps(summary, indent=2))
                    except Exception:
                        pass
        except Exception:
            pass

        # Refresh displays to show predictions and switch to Predictions tab
        self._update_games_display()
        try:
            if hasattr(self, 'notebook'):
                # Find the index of the Predictions tab and select it
                for i in range(self.notebook.index('end')):
                    if self.notebook.tab(i, 'text').startswith('🎯'):
...
For questions or issues, check the logs and API status."""
    ...
    def _update_available_bets(self):
        ...
        if hasattr(self, 'predictions') and self.predictions:
            ...
            # No mock/sample bets per user preference; leave empty when no real predictions
            pass
            
    def _update_performance_metrics(self):
        """Update performance metrics display"""
        # Only update if KPI labels exist; otherwise skip silently
        kpi_attrs = [
            'win_rate_value', 'roi_value', 'total_profit_value',
            'predictions_value', 'accuracy_value'
        ]
        if not all(hasattr(self, a) for a in kpi_attrs):
            return
        # Populate with real metrics when available; skip placeholders
        # Example (guarded) updates shown below; replace with real tracker data when wired
        try:
            self.win_rate_value.config(text="")
            self.roi_value.config(text="")
            self.total_profit_value.config(text="")
            self.predictions_value.config(text="")
            self.accuracy_value.config(text="")
        except Exception:
            return
    ...
    def _start_background_tasks(self):
        ...
        # Kick an initial data load shortly after launch
        self.root.after(500, self._refresh_all_data)
