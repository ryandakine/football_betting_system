...
class MCPAlertSystem:
    ...
    async def send_agent_metric_spike(self, game: Dict[str, Any], phantom_flag: float, scandal: float):
        """Send alert when agent metrics spike beyond thresholds."""
        try:
            title = f"Agent Metric Spike: {game.get('away_team','?')} @ {game.get('home_team','?')}"
            msg = f"phantom_flag={phantom_flag} scandal={scandal}"
            await self.send_push_notification(title, msg)
            ...
            logger.error(f"Error sending agent metric spike alert: {e}")
