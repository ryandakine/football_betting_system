    ...
    async def _parse_espn_games(self, events: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for event in events:
            try:
                ...
                # Get weather data for this stadium
                city = stadium_cities.get(stadium)
                weather_data = None
                if city:
                    weather_data = await self.get_game_weather(stadium, city)
                
                # Format weather for display
                if weather_data:
                    weather_str = f"{weather_data['conditions'].title()}, {weather_data['temperature']:.0f}°F"
                else:
