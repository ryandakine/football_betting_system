#!/usr/bin/env python3
"""
Zephyr NCAA/College Football AI Integration
============================================

Optional Zephyr model interface tailored for the NCAA betting system.
...
class ZephyrNCAAAdvisor:
    """
    Async client for Zephyr-powered NCAA/College Football insights.

    The advisor accepts raw game metadata and returns structured signals
    that can be merged into the unified NCAA workflow. When credentials
    ...
    def __init__(self, *, config: Optional[ZephyrConfig] = None) -> None:
        ...
        if not self.is_enabled:
            logger.info("Zephyr NCAA Advisor disabled (missing API key or base URL)")
    ...
    def _build_payload(self, game_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        system_prompt = (
            "You are an elite NCAA/College Football betting strategist with deep expertise in "
            "conference dynamics, rivalry games, home field advantage, and college-specific patterns. "
            "Provide concise, actionable insight based strictly on the supplied game metadata. "
            "Include confidence (0-1) and note risk factors such as injuries, weather, rivalries, "
            "conference championship implications, bowl eligibility, or market volatility when relevant. "
            "Consider that college football has more variability than NFL due to player inexperience, "
            "emotional factors in rivalry games, and significant home field advantages."
        )

        # Extract college-specific data
        is_rivalry = game_data.get("is_rivalry", False)
        is_conference_game = game_data.get("is_conference_game", True)
        home_advantage = game_data.get("home_advantage_score", "Unknown")
        ...
        user_prompt = json.dumps(
            {
                ...
                "weather_conditions": game_data.get("weather_conditions", "Unknown"),
                "sport_type": "ncaaf",
            },
            indent=2,
            sort_keys=True,
        ...
        return {
            ...
            "messages": [
                ...
                {
                    "role": "user",
                    "content": f"Analyze the following NCAA/College Football matchup and recommend a betting posture:\n{user_prompt}",
                },
            ],
    ...
    def _parse_response(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "injury" in lowered or "injured" in lowered:
            ...
            if "rivalry" in lowered:
                parsed["risk_flags"].append("RIVALRY_GAME")
            if "emotional" in lowered or "upset" in lowered:
                parsed["risk_flags"].append("UPSET_POTENTIAL")
            if "home" in lowered and ("advantage" in lowered or "crowd" in lowered):
                parsed["risk_flags"].append("HOME_FIELD_ADVANTAGE")
            if "conference" in lowered and "championship" in lowered:
                parsed["risk_flags"].append("PLAYOFF_IMPLICATIONS")
...
__all__ = ["ZephyrNCAAAdvisor", "ZephyrConfig"]
