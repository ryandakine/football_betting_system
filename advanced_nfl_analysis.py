...
class AdvancedNFLAnalysis:
    ...
    def __init__(self):
        ...
        # Current NFL games - will be populated by async method
        self.current_games = []
    ...
    async def analyze_game_comprehensive(self, game: Dict) -> Dict[str, Any]:
        ...
        if 'hrm' in self.models:
            try:
                ...
                analysis['failed_models'].append('hrm')
        ...
        if 'chemistry' in self.models:
            try:
                ...
                analysis['failed_models'].append('chemistry')
        ...
        if 'ensemble' in self.models:
            try:
                ...
                analysis['failed_models'].append('ensemble')

        # Only generate recommendations if all models succeeded
        if not analysis['failed_models']:
            analysis['betting_recommendations'] = self._generate_betting_recommendations(analysis)
        else:
            logger.warning(f"Skipping recommendations - models failed: {analysis['failed_models']}")
    ...
    def _format_game_for_hrm(self, game: Dict) -> str:
        ...
        weather_data = game.get('weather', 'Unknown')
        if isinstance(weather_data, dict):
            weather_info = f"{weather_data.get('condition', 'Unknown')}, {weather_data.get('temperature_f', 'N/A')}°F"
        else:
